...
import { getBlogPostsByCategory, getAllCategories, formatDate, getSafeImageUrl, decodeHtmlEntities } from '@/lib/wordpress-sgcf';
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const categoryName = posts[0]?.articleCategories?.nodes?.find(c => c.slug === slug)?.name || slug;
      ...
      <div className="container-counseling mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-3xl sm:text-4xl font-bold font-serif mb-4" style={{ color: 'var(--foreground)' }}>
            Category: <span className="text-gradient-primary">{categoryName}</span>
          </h1>
